...
for mat_folder in mat_path_folders:
    if mat_folder in ['TMH_NPH', ]:
        ...
        rv_dys = []
        for mat_i in range(len(label_event)):
            label_PESI_idx = labels_df_s['PESI'] == PESI_list[mat_i]
            label_event_idx = labels_df_s['Death'] == label_event[mat_i]
            label_time_idx = labels_df_s['follow_up_day'] == round(label_time[mat_i])
            label_idx = label_PESI_idx * label_time_idx * label_event_idx
            label_id = np.nonzero(label_idx.values)[0][0]
            rv_dys.append(labels_df_s.iloc[label_id]['RV_Dys'])
        ...
        for mod in ['fuse', 'fusePESI']:
            ...
            rv_idx = np.array(rv_dys)
            rv_idx_sorted = rv_idx[risk_order]
            rv_order_sorted = np.where(rv_idx_sorted)
            rv_risk_sorted = risk_sorted[rv_order_sorted]
            rv_x_sorted = risk_x[rv_order_sorted]
            ...
            plt.plot(rv_x_sorted, rv_risk_sorted, 'd' , color='orange', label='RV Dysfunction')
            plt.bar(rv_x_sorted, rv_risk_sorted,  width=1, color='orange')

            ax.set(
                title='RV Dysfunction Risk Distribution',
